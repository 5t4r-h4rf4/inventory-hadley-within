...
@section('title', 'Tambah Data Permintaan')
@section('header_title', 'Tambah Data Permintaan')
@section('header_subtitle', 'Masukkan data permintaan produk bulanan baru')
    ...
    <div class="col-lg-8 mx-auto">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <span>Form Tambah Data Permintaan</span>
                ...
                <form action="{{ route('demands.store') }}" method="POST">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6 mb-3 mb-md-0">
                            ...
                            <select name="month" id="month" class="form-select @error('month') is-invalid @enderror" required>
                                ...
                                @foreach($months as $val => $name)
                                    <option value="{{ $val }}" {{ old('month') == $val ? 'selected' : '' }}>
                                        {{ $name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="year" id="year" class="form-control @error('year') is-invalid @enderror" min="2020" max="2035" value="{{ old('year', date('Y')) }}" required>
                            @error('year')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" min="0" name="jumlah_permintaan" id="jumlah_permintaan" class="form-control @error('jumlah_permintaan') is-invalid @enderror" value="{{ old('jumlah_permintaan') }}" placeholder="Contoh: 150" required>
                        @error('jumlah_permintaan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <div class="form-text">Masukkan jumlah penjualan/permintaan aktual produk dalam periode bulan ini.</div>
                    </div>
