...
@section('header_subtitle', 'Perbarui data produk plastik')
            ...
            <div class="card-body">
                <form action="{{ route('products.update', $product->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="kode_produk" id="kode_produk" class="form-control @error('kode_produk') is-invalid @enderror" value="{{ old('kode_produk', $product->kode_produk) }}" required>
                        @error('kode_produk')
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" step="0.01" min="0" name="harga_produk" id="harga_produk" class="form-control @error('harga_produk') is-invalid @enderror" value="{{ old('harga_produk', $product->harga_produk) }}" required>
                            @error('harga_produk')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-text">Masukkan harga produk per bal (angka tanpa titik pemisah).</div>
                    </div>

                    <div class="row mb-4">
                        <div class="col-md-6">
                            <label for="barang_masuk" class="form-label fw-semibold text-dark">Barang Masuk</label>
                            <input type="number" min="0" name="barang_masuk" id="barang_masuk" class="form-control @error('barang_masuk') is-invalid @enderror" value="{{ old('barang_masuk', $product->barang_masuk) }}" required>
                            @error('barang_masuk')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" min="0" name="barang_keluar" id="barang_keluar" class="form-control @error('barang_keluar') is-invalid @enderror" value="{{ old('barang_keluar', $product->barang_keluar) }}" required>
                            @error('barang_keluar')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        <a href="{{ route('products.index') }}" class="btn btn-outline-secondary px-4" style="border-radius: 10px;">Batal</a>
